...
With the area as an `sf` object we can finally create the `rtoi` object. But before creating the `rtoi` we have to define a place to store it and the path to the *database* that will contain the downloaded files. For this example we already provide a *database* folder with the images to speed up the download process. Once those paths are defined we can proceed to the creation of the `rtoi` with the `new_rtoi` function. This function takes three parameter, the first one is the name, this name will be used to create the folder that stores it and to load it at a latter date. The second parameter is the region as an `sf` object. Finally the last parameter is the path to a folder.

```{r eval=FALSE, message=FALSE}

set_database("/home/ugaitz.amozarrain/DB2")
rtoi.path <- file.path("/home/ugaitz.amozarrain/regions")
...
rsat_derive(volcano, product = "S2MSI2A", variable = "redModified", fun = redModified, overwrite=TRUE)

```

Once we have created the new variable, we can use it as a band. In order to load it we would use the `rsat_get_SpatRaster` function, but instead of using the band name we specify the variable name defined earlier. We will also load the blue and green bands to use them together to create an RGB image.
