...
database.dir <- file.path(wd,"DATABASE")
...
```{r basic_rtoi}
filomena <- new_rtoi("filomena", roi, regions.dir, database.dir)
```
...
Import the results into `R` using the `rsat_get_SpatRaster()` function. The numbers must be truncated between $-1$ and $1$ to avoid values outside the feasible range (sun reflections on mirror-like surfaces, such as water, can lead to misleading results):
...
`rsat` package also allows us to fill missing values in images. `rsat_smoothing_images` is the implementation of a spatio temporal method called image mean anomaly (IMA) for gap filling and smoothing satellite data (Militino et al. 2019).
