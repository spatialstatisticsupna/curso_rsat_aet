...
![The 3 parts of rtoi](imgs/rtoi_structure.png){width="322"} \## Basic `rtoi` functions
...
madrid <-new_rtoi("madrid",madrid.sf,"./regions","./DATABASE")
print(madrid)
```
...
madrid <- read_rtoi("./regions/madrid")
print(madrid)
```
...
plot(madrid,"preview",dates=dates(madrid)[1:9],product="LANDSAT_8_C1",compass.rm=T)
```
...
The `rtoi` object consists of three parts, the database address, the object loaded in R and the folder with all the processed data of the region of interest. The three parts are related to each other. When we download images, they are stored in the database, but the `rtoi` folder is still empty (it only has meta data). However, when we run the mosaic function, the image information is extracted from the database and processed to generate a single image from the region of interest.
...
![2 people working with the same database](imgs/rtoi_db.png) \## Basic processing
...
```{r basic_paths}
db.path <- "D:/curso_rsat_aet/DATABASE"
rtoi.path <- "D:/curso_rsat_aet/regions"
dir.create(db.path, recursive = TRUE, showWarnings = FALSE)
dir.create(rtoi.path, recursive = TRUE, showWarnings = FALSE)
```

Then, create a new `rtoi` using the previous information:

```{r basic_rtoi}
filomena <- new_rtoi("filomena", roi, rtoi.path, db.path)
```
...
```{r basic_variables}
show_variables()
d.list <- rsat_list_data(filomena)
# unlist to visualize
for (c in 1:ncol(d.list) ) {
  d.list[,c] <- unlist(d.list[,c])
}
d.list
```
...
```{r basic_mask}
clds.msk <- rsat_get_SpatRaster(filomena, "mod09ga", "CloudMask")
```
...
clds.msk <- resample(clds.msk, ndsi.img, method = "near")
```
...
`rsat` package also allows us to fill missing values in images.

```{r basic_ndsi_IMA}
rsat_smoothing_images(snow.spain,"IMA")
```
